...
link_extension()
{
    ...
    ln -sf "${SRCDIR}/filament_dryer.py" "${KLIPPER_PATH}/klippy/extras/filament_dryer.py"
    echo "[OK]"
}
...
# Add updater for filament_dryer to moonraker.conf
add_updater()
{
    echo -e -n "Adding update manager to moonraker.conf... "

    update_section=$(grep -c '\[update_manager klipper_filament_dryer\]' ${MOONRAKER_CONFIG_DIR}/moonraker.conf || true)
    if [ "${update_section}" -eq 0 ]; then
        ...
        echo -e "[update_manager klipper_filament_dryer] already exists in moonraker.conf [SKIPPED]"
    fi
}
...
uninstall()
{
    if [ -f "${KLIPPER_PATH}/klippy/extras/filament_dryer.py" ]; then
        echo -n "Uninstalling... "
        rm -f "${KLIPPER_PATH}/klippy/extras/filament_dryer.py"
        echo "[OK]"
        echo "You can now remove the [update_manager klipper_filament_dryer] section in your moonraker.conf and delete this directory. Also remove all [print_setting] configurations from your Klipper configuration."
    else
        echo "filament_dryer.py not found in \"${KLIPPER_PATH}/klippy/extras/\". Is it installed?"
        echo "[FAILED]"
    fi
}
